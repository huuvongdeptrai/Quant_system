# PHIÊN BẢN BASELINE - Không chỉnh sửa - Dùng để backtest so sánh
import pandas as pd
import numpy as np
import sys
...
class ICTZonesProStrategyV1(IStrategy):
    """
    ...
    def analyze(self, df: pd.DataFrame, macro_context: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if len(vol_window) == self.ob_length*2 + 1:
                ...
                if is_phv:
                    ...
                    if os_state == 1: # Bull Order Block
                        ob_top = (df['high'].iloc[i-self.ob_length] + df['low'].iloc[i-self.ob_length]) / 2 
                        ...
                        ob_btm = (df['high'].iloc[i-self.ob_length] + df['low'].iloc[i-self.ob_length]) / 2 
        ...
        def check_pa(is_bull: bool) -> bool:
            ...
            if is_bull:
                is_pinbar = wBtm > body * 2.0 and wTop < body
            else:
                is_pinbar = wTop > body * 2.0 and wBtm < body
        ...
        if l_score >= self.min_confluence:
            action = "BUY"
        ...
        elif s_score >= self.min_confluence:
            action = "SELL"
